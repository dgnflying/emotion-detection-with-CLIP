...
parser.add_argument(
    "--average",
    "-a",
    help="Display the average vector for each emotion",
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--all",
    "-A",
    help="Display all embeddings for every emotion",
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--text",
    "-t",
    help="Display the text embeddings of the emotions",
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--comparison",
...
parser.add_argument(
    "--titles",
    "-T",
    help="Display the titles of the plots",
    action=argparse.BooleanOptionalAction,
)
...
COLORS = {
    ...
    "disgust": {
        "text": sns.light_palette("green")[3],
        "image": sns.dark_palette("green")[3],
    },
    "fear": {
        "text": sns.light_palette("purple")[3],
        "image": sns.dark_palette("purple")[3],
    },
    "happy": {
        "text": sns.light_palette("yellow")[3],
        "image": sns.dark_palette("yellow")[3],
    },
    "neutral": {
        "text": sns.light_palette("gray")[3],
        "image": sns.dark_palette("gray")[3],
    },
    "sad": {"text": sns.light_palette("blue")[3], "image": sns.dark_palette("blue")[3]},
    "surprise": {
        "text": sns.light_palette("orange")[3],
        "image": sns.dark_palette("orange")[3],
    },
}
...
if __name__ == "__main__":
    ...
    if ARGS.text:
        ...
        text_data = TSNE(random_state=0, verbose=1, perplexity=5).fit_transform(
            text_vecs
        )
        ...
        for i, emotion in enumerate(EMOTIONS):
            ...
            plt.plot(
                [0, text_data[indices, 0][0]],
                [0, text_data[indices, 1][0]],
                color=color_text,
            )
            if ARGS.titles:
    ...
    if ARGS.comparison:
        ...
        for i, emotion in enumerate(EMOTIONS):
            ...
            plt.scatter(
                text_data[text_indices, 0],
                text_data[text_indices, 1],
                label=f"{emotion.capitalize()} (Text)",
                color=color_text,
            )
            plt.plot(
                [0, text_data[text_indices, 0][0]],
                [0, text_data[text_indices, 1][0]],
                color=color_text,
            )
            plt.scatter(
                avg_img_vec[0],
                avg_img_vec[1],
                label=f"{emotion.capitalize()} (Image)",
                color=color_image,
            )
